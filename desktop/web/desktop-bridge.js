...
(function(){
  ...
  let paired=false,todoSyncTimer=null;
  ...
  function taskForSync(t,done){const out=JSON.parse(JSON.stringify(t||{}));delete out.notionId;out.id=String(out.id||('desktop-'+Date.now()+'-'+Math.random().toString(36).slice(2,7)));out.done=!!done;out.source='windows';if(done){out.completedAt=Number(out.completedAt)||Date.now()}else delete out.completedAt;return out;}
  async function syncTodoState(raw){if(!paired||!raw)return;let state=null;try{state=typeof raw==='string'?JSON.parse(raw):raw}catch(e){return}const active=Array.isArray(state?.active)?state.active:[],archive=Array.isArray(state?.archive)?state.archive:[];for(const t of active.slice(0,250)){try{await request('/api/tasks','POST',taskForSync(t,false))}catch(e){}}for(const t of archive.slice(0,250)){try{await request('/api/tasks','POST',taskForSync(t,true))}catch(e){}}}
  function scheduleTodoSync(raw){clearTimeout(todoSyncTimer);todoSyncTimer=setTimeout(()=>syncTodoState(raw),900)}
...
`;document.head.appendChild(s)}
  function ensurePairUi(){ensurePairStyle();if(document.getElementById('desktopPairSheet'))return;const sheet=document.createElement('div');sheet.id='desktopPairSheet';sheet.innerHTML='<div class="dpsCard"><div style="font-size:9px;letter-spacing:.14em;color:#9db0ff;font-weight:900">HOME DESKTOP</div><h2>Pair with HOME Sync</h2><p>Paste the same HOME token used by your phone. Windows stores it in Credential Manager, not in this page.</p><input id="desktopPairToken" type="password" autocomplete="off" placeholder="HOME token"><div class="dpsActions"><button class="dpsCancel">Cancel</button><button class="dpsSave">Pair</button></div><p class="dpsNote">Without pairing, HOME still works locally. Pairing syncs tasks, To-Do notes/details, behavioural telemetry and written learning attempts.</p></div>';document.body.appendChild(sheet);sheet.querySelector('.dpsCancel').onclick=()=>sheet.classList.remove('show');sheet.querySelector('.dpsSave').onclick=async()=>{const input=sheet.querySelector('#desktopPairToken'),token=input.value.trim();if(token.length<16){input.focus();return}try{await invoke('home_set_token',{token});paired=true;sheet.classList.remove('show');input.value='';updateStatus();scheduleTodoSync(localStorage.getItem('todoState')||'');window.onNotionConnected?.();await flushActivity()}catch(e){alert('Could not pair HOME: '+String(e))}};
  ...
  window.Native={
    ...
    saveState:(key,value)=>{const raw=String(value??'');localStorage.setItem(key,raw);if(key==='todoState')scheduleTodoSync(raw);return true},
    ...
    requestNotionSync:async()=>{if(!paired){showPair();return}try{await syncTodoState(localStorage.getItem('todoState')||'');const data=await request('/v1/snapshot');window.onNotionSnapshot?.({open:(data.open||[]).map(normalizeTask),completed:(data.completed||[]).map(normalizeTask)})}catch(e){window.onNotionSyncError?.({message:String(e)})}},
    createNotionTask:async title=>{if(!paired){showPair();return}try{const t=normalizeTask(await request('/api/tasks','POST',{title:String(title||''),area:'Personal',minutes:0,note:'',done:false,source:'windows'}));window.onNotionTaskCreated?.(t)}catch(e){window.onNotionSyncError?.({message:String(e)})}},
  ...
  window.AdaptiveNative={
    ...
    logPrivateActivity:raw=>{let row={};try{row=JSON.parse(raw||'{}')}catch(e){}const item={...row,source:'windows',createdAt:new Date(Number(row.at)||Date.now()).toISOString()};if(paired)request('/api/activity','POST',item).catch(()=>queueActivity(item));else queueActivity(item)},
  ...
  document.addEventListener('DOMContentLoaded',async()=>{await refreshPairState();ensurePairUi();updateStatus();if(paired){scheduleTodoSync(localStorage.getItem('todoState')||'');setTimeout(()=>{window.onNotionConnected?.();flushActivity()},900)}});
})();
