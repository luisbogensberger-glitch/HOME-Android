/* V-Brain Patch v10 — broad declarative live UI control without remote code execution. */
(function(){
  'use strict';
  if(window.__VBRAIN_PATCH_V10__)return;window.__VBRAIN_PATCH_V10__=true;
  const DEFAULT={
    refreshMs:120000,
    home:{order:['calendar','todos','tube','gym'],visible:{calendar:true,todos:true,tube:true,gym:true},gap:14,minHeight:132,scale:1,heroImage:'',heroPosition:'center'},
  ...
  let cfg=JSON.parse(JSON.stringify(DEFAULT)),timer=0;
  ...
  function applyHome(c){const grid=document.querySelector('#homeScreen .homeGrid');if(grid){grid.style.gap=clamp(c.gap,6,28)+'px';const order=Array.isArray(c.order)?c.order.filter(x=>['calendar','todos','tube','gym'].includes(x)):DEFAULT.home.order;for(const k of order){const el=card(k);if(el)grid.appendChild(el)}}for(const k of ['calendar','todos','tube','gym']){const el=card(k);if(!el)continue;el.style.display=c.visible?.[k]===false?'none':'';el.style.minHeight=clamp(c.minHeight,104,210)+'px';el.style.setProperty('--vbrain-card-scale',String(clamp(c.scale,.94,1.06)));const img=safeImage(window.HOMEAdaptive?.config?.home?.cards?.[k]?.image||'');if(img){el.style.backgroundImage=`linear-gradient(180deg,rgba(8,10,14,.10),rgba(8,10,14,.48)),url("${img.replace(/["'()]/g,encodeURIComponent)}")`;el.style.backgroundSize='cover';el.style.backgroundPosition=str(window.HOMEAdaptive?.config?.home?.cards?.[k]?.position||'center',40)}}const hero=document.querySelector('#homeScreen .homeHero'),hi=safeImage(c.heroImage);if(hero&&hi){hero.style.backgroundImage=`linear-gradient(180deg,rgba(17,18,20,.03),rgba(17,18,20,.15)),url("${hi.replace(/["'()]/g,encodeURIComponent)}")`;hero.style.backgroundPosition=str(c.heroPosition||'center',40)}}
  ...
  function apply(){cfg=merge(DEFAULT,current());cfg.refreshMs=clamp(cfg.refreshMs,30000,600000);applyHome(cfg.home);applyScore(cfg.score);applyBrain(cfg.brain);applyMotion(cfg.motion);applyBanner(cfg.banner);document.documentElement.dataset.vbrainPatch='10';return cfg}
  ...
  window.VBrainPatch={version:10,apply,refresh,config:()=>JSON.parse(JSON.stringify(cfg))};
  apply();setTimeout(refresh,1200);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refresh()});
})();
