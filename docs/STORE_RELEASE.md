...
1. **Client app** — local UI, local cache and per-user authentication session. Android stores session material encrypted with Android Keystore.
...
- WebView universal file-origin access remains disabled.
...
- Complete Google Play Data safety from the *actual Store build*, not the private build.
...
- Run penetration/security tests, dependency scanning, crash tests and store pre-launch reports on the release artifact.
...
The current repository is an Android application, so an App Store submission also requires a separate iOS target. Reuse the same Store API, HOME account, user-isolated data model, privacy/deletion endpoints and OpenAI backend. Bundle executable UI logic with the iOS binary; do not use the private Android remote-code mechanism. Android-only notification/accessibility features cannot be treated as cross-platform core functionality.
