...
function consentPage(env: Env) {
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Connect Veqrya</title><style>
  ...
  </style></head><body><main class="card"><div class="label">Veqrya · Personal OS</div><h1>Connect Veqrya</h1><p class="sub">Give ChatGPT or Codex access to the Veqrya account you choose.</p>
  ...
  <section id="consent-panel" hidden><div class="box"><div class="label">Application</div><div id="client-name" class="value"></div></div><div class="box"><div class="label">Redirect</div><div id="client-uri" class="value"></div></div><div class="box"><div class="label">Requested permissions</div><ul id="scopes"></ul></div><p>Veqrya will only expose data belonging to this signed-in Veqrya account. You can revoke access later.</p><div class="row"><button id="deny" class="danger">Deny</button><button id="approve" class="primary">Allow access</button></div><button id="sign-out" style="width:100%;margin-top:10px;background:transparent">Use another account</button></section>
  <p id="message"></p><p class="fine">By continuing you agree to the Veqrya terms and privacy policy. The public plugin does not read WhatsApp, notifications, contacts, or precise location.</p></main><script>window.HOME_AUTH_CONFIG=${config}</script><script type="module" src="/app.js"></script></body></html>`
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/health') return new Response(JSON.stringify({ok:true,service:'veqrya-auth-fallback'}),{headers:{'content-type':'application/json','cache-control':'no-store'}})
